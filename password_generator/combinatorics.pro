...
generate_binomial( [], [] ).
generate_binomial( [ X|Xs ], [ X|Ys ] ) :- generate_binomial( Xs, Ys ).
generate_binomial( [ _|Xs ], Ys ) :- generate_binomial( Xs, Ys ).
...
disposition( Xs, Ys ) :- permutation( Xs, Xs, Ys ).

atomics( [], '' ) :- !.
atomics( Xs, Ys ) :- atomic_list_concat( Xs, Ys ).

remove_atom( [], [] ).
remove_atom( [ X|Xs ], [ X|Ys ] ) :- var( X ),
remove_atom( Xs, Ys ).
remove_atom( [ X|Xs ], Ys ) :- \+var( X ),
remove_atom( Xs, Ys ).
